...
(                                                            -- атрибуты сущности, т.е столбцы в таблице
    ...
    is_deleted    BIT DEFAULT 0 COMMENT 'Существует ли пользователь',
    INDEX users_lastname_firstname_idx (lastname, firstname) -- то, по чему чаще всего будут искать в БД: фамилия и имя
);

-- 1:1
DROP TABLE IF EXISTS `profiles`; -- `` указываются, если считаешь, что есть таблицы с таким же именем в системных БД
